...
function encontrarSalaDoConvidado(idConvidado) {
    let salaEncontrada;
    
    Object.keys(jogo.salas).forEach(sala => {
      if (sala.idConvidado === idConvidado) {
        console.log(sala.nomeDaSala,"ITS");
        salaEncontrada = sala;
      }
    });
    console.log("A sala do convidado foi encontrada!");
...
sockets.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        if(Object.keys(jogo.salas).length >= 1){
            if(jogo.salas[socket.id]){
                ...
                sockets.in(jogo.salas[socket.id]).disconnectSockets();
    ...
    socket.on('iniciarJanelaDeJogo', function () {
        sockets.to(jogo.salas[socket.id].idDono).emit('iniciarJogo');
        sockets.to(jogo.salas[socket.id].idConvidado).emit('iniciarJogo');
